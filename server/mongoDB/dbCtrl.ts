import { ICreateResponse, IDatabaseError, IDatabaseResponse, IReadResponse, IUpdateResponse } from './interface';
import App from '../app';
import { Cursor, MongoCallback, MongoClient, MongoError, ObjectID } from 'mongodb';
...
export namespace dbCtrl {
  ...
  export function readAll(collectionName: string, cb: (dbResponse: IDatabaseResponse<IReadResponse[]>) => void): void {
    ...
    App.db.collection(collectionName, (err, collection) => {
      ...
      collection.find({}, (innerError, cursor) => {
        ...
        if (innerError) {
          console.log("hier ist der scheis")
          return cb({
            error: innerError
          });
        }
  ...
  export function remove(id: string, collectionName: string, cb: (dbResp: IDatabaseResponse<any>) => void): void {
    ...
    App.db.collection(collectionName, (err, collection) => {
      ...
      if (err) {
        return cb({ error: mongoErrorToGeneralDbError(err) });
      }

      collection.deleteOne({ '_id': new ObjectID(id) }, (innerError, result) => {

        if (innerError) {
          return cb({ error: mongoErrorToGeneralDbError(innerError) });
        }
  ...
  function mongoErrorToGeneralDbError(err: MongoError): IDatabaseError {
